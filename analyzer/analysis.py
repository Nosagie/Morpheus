...
def generate_signal(period,crypto_pairs):
    result = {}

    for pairData in crypto_pairs:
        # get period data from database
        pairData = CryptoPairs.objects.get(pair=pairData)
        periodData = PeriodData.objects.filter(crypto_pair=pairData)
        bids,asks,prices = [],[],[]

        # ensure all previous periods are accounted for
        all_period_sigs = PeriodSignals.objects.filter(pair=pairData)

        if len(all_period_sigs) < period and period > 0:
            for i in range(0,period+1):
                generate_signal(period-1,[pairData])
        ...
        # create new period
        unique_id = str(pairData.pair) + str(period)
        new_period = PeriodSignals(identifier=unique_id,pair=pairData,period=period,price=prices[period])
        
        try:
            new_period.save()
        except IntegrityError:
            new_period = PeriodSignals.objects.get(identifier=unique_id)
            new_period.price = prices[period]
            new_period.save()
        ...
        if period < 2:
            ...
            prev_period_data = PeriodSignals.objects.get(identifier=str(pairData.pair)+str(period-1))
            prev_prev_period_data = PeriodSignals.objects.get(identifier=str(pairData.pair)+str(period-2))
            ...
            try:
                ...
                new_period.cumulative_score = None
        ...
        if delay_1 >= 0 and delay_2 >= 0:
            ...
            delay2_period_data = PeriodSignals.objects.get(identifier=str(pairData.pair)+str(delay_2))
            ...
            if new_period.param1 and new_period.param2 and new_period.param3:
                new_period.action = GO_SIGNAL
            else:
                new_period.action = STOP_SIGNAL
            new_period.save()


        result[pairData.pair] = new_period.action 
    return result 
